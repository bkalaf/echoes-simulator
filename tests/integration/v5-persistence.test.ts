...
describe("V5 persistence and replay boundaries", () => {
  ...
  it("round-trips BigInt durable state, causal events, manifests, and noncausal labels", () => {
    ...
    store.saveV5RunManifest(manifest); store.appendV5CausalEvents("RUN_V5", [event]); const saved = store.saveV5Checkpoint("RUN_V5", state, v5EventHistoryHash([event])); store.saveV5Labels("RUN_V5", 5, { S1: "Accepted Name" });
  ...
  it("accepts one exact persisted blocking naming batch without changing causal state", () => {
    ...
    const response = { schemaVersion: "echoes-v5-naming-batch-response-v1" as const, batchId: batch.batchId, runId: batch.runId, decisions: batch.items.map((request) => ({ requestId: request.requestId, entityType: request.entityType, entityId: request.entityId, label: `Name ${request.entityId}` })) };
    const validated = validateNamingBatchResponseV5(batch, response);
    expect(validated.accepted).toBe(true);
    store.acceptV5NamingRequests("RUN_NAMES", response.decisions, 5);
  ...
  it("accepts a deterministic non-blocking naming batch without changing run status", () => {
    ...
    store.acceptV5NamingRequests("RUN_BATCHED_NAMES", batch.items.map((request) => ({ requestId: request.requestId, entityId: request.entityId, label: `Name ${request.entityId}` })), batch.year, "BATCHED");
