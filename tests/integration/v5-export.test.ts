...
import { legacyImportTestCanonicalAuthorityV5 } from "../../src/core/v5/canonical-adapter.js";
...
describe("V5 persisted read/export compatibility", () => {
  it("exports a completed V5 checkpoint through the existing verified ZIP contract", () => {
    ...
    try {
      const run = runPersistedV5Diagnostic({ store, canonicalAuthority: legacyImportTestCanonicalAuthorityV5(resolve("resources/canonical")), normalizedSeed: normalizeSeed("V5_EXPORT_FIXTURE"), throughYear: 0 });
      expect(run.status).toBe("COMPLETE");
      const generated = buildPersistedV5Export(store, run.runId);
