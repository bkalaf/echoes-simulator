...
describe("R19_B01 complete research gate", () => {
  ...
  it("locks parrot cognition, cultural transmission, inward Mythos mappings, and batch order", () => {
    ...
    expect(architecture.completedRegionBatches.slice(-2)).toEqual(["R18_B01","R19_B01"]); expect(architecture.completedRegionBatches).toHaveLength(21); expect(architecture.status).toBe("ARCHITECTURE_LOCKED_RESEARCH_IN_PROGRESS");
  });
});
