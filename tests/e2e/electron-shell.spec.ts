...
test("launches with the current persisted V4-transition preflight and naming barrier", async () => {
  ...
  try {
    ...
    await expect(page.getByText("RETIRED_V3_AUTHORITY", { exact: true })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: /^NAMING_JOB_/ })).toBeVisible();
