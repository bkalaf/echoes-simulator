...
import { buildNamingJob, type NamingContext, type NamingJob } from "../naming/naming.js";
...
export interface CanonicalHistorySite { siteId: string; regionId: string; currentSiteName: string; nameStatus: string; classification: string; attractivenessTier: string; unnamedPois?: NamingContext["unnamedPois"]; }
...
export interface CanonicalMigrationYearSummary {
  world: WorldKey; year: number; transferCount: number; population: string; exactRowsSha256: string;
  routes: { fromSettlementId: string; toSettlementId: string; population: string; transferCount: number }[];
}
export type CanonicalHistoryYearChunk = Pick<CanonicalHistoryResult, "events" | "migrations" | "founding" | "djt" | "governmentEpochs" | "economicEpochs" | "social" | "institutions"> & { migrationSummaries: CanonicalMigrationYearSummary[] };
export interface CanonicalHistoryInput {
  ...
  autoAcceptNaming: boolean; checkpointInterval?: number; onCheckpoint?: (world: CanonicalHistoryWorld) => void; onHistoryYear?: (year: number, chunk: CanonicalHistoryYearChunk) => void;
}
...
type CohortMergeGroup = { first: Cohort; population: bigint; weightedWealth: bigint; createdYear: number; count: number };
function addMergedCohort(groups: Map<string, CohortMergeGroup>, cohort: Cohort): void {
  if (cohort.population <= 0n) return;
  const key = `${cohort.worldKey}\0${cohort.settlementId}\0${cohort.breedId}`;
  const group = groups.get(key);
  if (!group) { groups.set(key, { first: cohort, population: cohort.population, weightedWealth: BigInt(cohort.wealthScore) * cohort.population, createdYear: cohort.createdYear, count: 1 }); return; }
  group.population += cohort.population;
  group.weightedWealth += BigInt(cohort.wealthScore) * cohort.population;
  group.createdYear = Math.min(group.createdYear, cohort.createdYear);
  group.count += 1;
  if (cohort.cohortId.localeCompare(group.first.cohortId) < 0) group.first = cohort;
}
function finishMergedCohorts(groups: Map<string, CohortMergeGroup>, year: number): Cohort[] {
  return [...groups.values()].map((group) => {
    const first = group.first;
    return { ...first, cohortId: `COHORT_${first.worldKey}_${digest([first.settlementId, first.breedId]).slice(0, 20)}`, population: group.population, wealthScore: Number(group.weightedWealth / group.population), createdYear: group.createdYear, originCohortId: group.count === 1 ? first.originCohortId : `MERGED_${year}`, createdByEventId: group.count === 1 ? first.createdByEventId : `EVENT_${first.worldKey}_${year}_COHORT_MERGE` };
...
function mergeCohorts(cohorts: readonly Cohort[], year: number): Cohort[] {
  const groups = new Map<string, CohortMergeGroup>();
  for (const cohort of cohorts) addMergedCohort(groups, cohort);
  return finishMergedCohorts(groups, year);
}

type MigrationRow = CanonicalHistoryResult["migrations"][number];
class MigrationYearAccumulator {
  private readonly hash = createHash("sha256");
  private readonly routes = new Map<string, { fromSettlementId: string; toSettlementId: string; population: bigint; transferCount: number }>();
  private count = 0;
  private population = 0n;
  constructor(readonly world: WorldKey, readonly year: number) { this.hash.update("["); }
  add(row: MigrationRow): void {
    if (this.count > 0) this.hash.update(",");
    this.hash.update(canonicalJson(row));
    this.count += 1; this.population += BigInt(row.population);
    const key = `${row.fromSettlementId}\0${row.toSettlementId}`;
    const route = this.routes.get(key) ?? { fromSettlementId: row.fromSettlementId, toSettlementId: row.toSettlementId, population: 0n, transferCount: 0 };
    route.population += BigInt(row.population); route.transferCount += 1; this.routes.set(key, route);
  }
  finish(): CanonicalMigrationYearSummary {
    this.hash.update("]");
    return { world: this.world, year: this.year, transferCount: this.count, population: this.population.toString(), exactRowsSha256: this.hash.digest("hex"), routes: [...this.routes.values()].sort((left, right) => left.fromSettlementId.localeCompare(right.fromSettlementId) || left.toSettlementId.localeCompare(right.toSettlementId)).map((route) => ({ ...route, population: route.population.toString() })) };
  }
}

function groupResidents(cohorts: readonly Cohort[]): Map<string, Cohort[]> {
  const grouped = new Map<string, Cohort[]>();
  for (const cohort of cohorts) if (cohort.population > 0n) {
    const residents = grouped.get(cohort.settlementId);
    if (residents) residents.push(cohort); else grouped.set(cohort.settlementId, [cohort]);
  }
  return grouped;
}
...
function migrationRate(destination: CanonicalHistorySettlement, semantic: EffectiveBreedSemantics, faction: Faction, sovereignFaction: Faction): number {
  let comparison = 0;
...
export function runCanonicalHistory(input: CanonicalHistoryInput): CanonicalHistoryResult {
  ...
  const breedFactionByWorld = new Map(WORLDS.flatMap((world) => input.semantics.map((semantic) => [`${world}\0${semantic.breedId}`, breedFaction(semantic, mapping, world)] as const)));
  ...
  for (let year = Math.min(...worlds.map((world) => world.year)) + 1; year <= input.yearEnd; year += 1) {
    const migrationByWorld = new Map(WORLDS.map((world) => [world, new MigrationYearAccumulator(world, year)]));
    for (const state of worlds) {
      const sovereign = input.sovereign[state.world];
      let settlementById = new Map(state.settlements.map((settlement) => [settlement.settlementId, settlement]));
      const before = total(state.cohorts);
      state.cohorts = state.cohorts.map((cohort) => {
        const settlement = settlementById.get(cohort.settlementId)!;
        ...
        const faction = breedFactionByWorld.get(`${state.world}\0${cohort.breedId}`)!;
      ...
      settlementById = new Map(state.settlements.map((settlement) => [settlement.settlementId, settlement]));
      ...
      const mergedCohorts = new Map<string, CohortMergeGroup>();
      for (const cohort of state.cohorts) {
        if (cohort.outboundMigrationNotBeforeYear !== null && year < cohort.outboundMigrationNotBeforeYear) { addMergedCohort(mergedCohorts, cohort); continue; }
        const destinations = edgesByOrigin.get(cohort.settlementId) ?? [];
        const proposals = destinations.map((edge) => { const destination = settlementById.get(edge.destinationId)!; const rateTwelfths = migrationRate(destination, semanticById.get(cohort.breedId)!, breedFactionByWorld.get(`${state.world}\0${cohort.breedId}`)!, sovereign.sovereignFaction); return { destination, rateTwelfths, proposed: cohort.population * BigInt(rateTwelfths) / 1200n }; });
        ...
        addMergedCohort(mergedCohorts, { ...cohort, population: cohort.population - scale });
        let applied = 0n;
        for (let index = 0; index < proposals.length; index += 1) {
          ...
          addMergedCohort(mergedCohorts, { ...cohort, cohortId: `${cohort.cohortId}__MIGRATION_${year}_${proposal.destination.settlementId}`, settlementId: proposal.destination.settlementId, population: amount, wealthScore: 0, createdYear: year, originCohortId: cohort.cohortId, createdByEventId: `EVENT_${state.world}_${year}_MIGRATION`, outboundMigrationNotBeforeYear: null });
          const migration = { world: state.world, year, breedId: cohort.breedId, fromSettlementId: cohort.settlementId, toSettlementId: proposal.destination.settlementId, population: amount.toString(), retainedWealth: cohort.wealthScore, migrantWealth: 0 as const, rateTwelfths: proposal.rateTwelfths };
          migrationByWorld.get(state.world)!.add(migration);
          if (!input.onHistoryYear) migrations.push(migration);
        }
      }
      state.cohorts = finishMergedCohorts(mergedCohorts, year);
    ...
    const migrationSummaries = WORLDS.map((world) => migrationByWorld.get(world)!.finish());
    if (input.onHistoryYear) {
      input.onHistoryYear(year, { events: [...events], migrations: [...migrations], migrationSummaries, founding: [...founding], djt: [...djt], governmentEpochs: [...governmentEpochs], economicEpochs: [...economicEpochs], social: [...social], institutions: [...institutions] });
      events.splice(0); migrations.splice(0); founding.splice(0); djt.splice(0); governmentEpochs.splice(0); economicEpochs.splice(0); social.splice(0); institutions.splice(0);
    }
    if (year % interval === 0 || year === input.yearEnd) for (const state of worlds) input.onCheckpoint?.(state);
    if (input.onHistoryYear) checkpoints.splice(0);
