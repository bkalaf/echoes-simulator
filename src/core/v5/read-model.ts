...
export function buildDivergenceReport(
  ...
  for (const siteId of settlementSiteIds) {
    ...
    items.push({ comparisonId: `MIGRATION/${siteId}`, category: "MIGRATION", classification: classify(migrationValues), worlds: Object.fromEntries(worlds.map((world, index) => [world, migrationValues[index]])), causeEventIds: [] });
  }
  ...
  for (const stateKey of stateKeys) {
    ...
    const institutionValues = worlds.map((world, index) => { const row = rows[index]; if (!row) return null; const institutions = states[world].institutions.filter((institution) => institution.stateId === row.stateId && institution.dissolvedYear === null); const institutionIds = new Set(institutions.map((institution) => institution.institutionId)); const offices = states[world].offices.filter((office) => institutionIds.has(office.institutionId)); const activeTerms = states[world].officeTerms.filter((term) => term.endYear === null && offices.some((office) => office.officeId === term.officeId)); return { types: institutions.map((institution) => institution.institutionType).sort(), officeCount: offices.length, filledCount: activeTerms.length }; });
