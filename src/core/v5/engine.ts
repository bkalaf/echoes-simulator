...
import { reviewAllSettlementsSocialMobilityV5 } from "./social.js";
...
import { establishResourceGeographyV5, reconcileDiplomacyAndConflictV5, updateCivicInstitutionsAndSecurityV5, updateIndustriesAndGuildsV5, validateSecurityForceOrganizationIntegrityV5 } from "./historical-dynamism.js";
import { applyEnclaveDemographyV5, applyPublicSliceGrowthModifiersV5, ensurePopulationSlicesV5, reconcilePublicPopulationSlicesV5, validatePopulationPartitionV5 } from "./population-slices.js";
...
export function applyTemporalEventsV5(state: WorldStateV5): { state: WorldStateV5; events: CausalEventV5[] } {
  ...
  if (deaths.length) {
    const deathIds = new Set(deaths); const successionOfficeIds = new Set<string>();
    working = {
      ...working,
      politicalPeople: working.politicalPeople.map((person) => deathIds.has(person.personId) ? { ...person, actualDeathYear: Math.min(person.actualDeathYear ?? person.naturalDeathYear, state.year) } : person),
      officeTerms: working.officeTerms.map((term) => { if (!officeTermActiveAt(term, state.year) || !deathIds.has(term.personId)) return term; successionOfficeIds.add(term.officeId); return { ...term, endYear: state.year, terminationReason: "DEATH" as const }; }),
    };
    events.push({ schemaVersion: "echoes-causal-event-v5", eventId: `EVT_${state.worldKey}_${state.year}_NATURAL_DEATHS`, worldKey: state.worldKey, year: state.year, phase: "TEMPORAL", sequence: 0, eventType: "PoliticalPeopleDied", entityType: "WORLD", entityId: state.worldKey, causeEventIds: [], mechanicsVersion: V5_MECHANICS_VERSION, causalDerivationVersion: V5_CAUSAL_DERIVATION_VERSION, keyedDecisionIdentity: null, mutations: [], payload: { personIds: deaths, successionOfficeIds: [...successionOfficeIds].sort(), aggregatePopulationChanged: false } });
  }
  ...
  const expiring = working.officeTerms.filter((term) => term.endYear === state.year && term.terminationReason === null).map((term) => term.officeTermId); const expiringIds = new Set(expiring);
  working.officeTerms = working.officeTerms.map((term) => expiringIds.has(term.officeTermId) ? { ...term, terminationReason: "TERM_EXPIRED" } : term);
...
function applyScheduled(state: WorldStateV5, context: V5EngineContext): { state: WorldStateV5; events: CausalEventV5[]; namingRequests: NamingRequestV5[] } {
  ...
  for (const transaction of transactions.filter((candidate) => candidate.type !== "CANONICAL_FOUNDING")) {
    ...
    else if (transaction.type === "HISTORICAL_CONFLICT_ACTION") { const resolved = executeHistoricalConflictActionV5({ state: working, canonical: context.canonical, ownerInputs: context.ownerInputs, mode: context.mode ?? "DIAGNOSTIC", action: transaction.action }); working = resolved.state; events.push(...resolved.events); }
...
function validateState(state: WorldStateV5): void {
  ...
  const activeStakesByOrganization = new Map<string, typeof state.ownershipStakes>(); for (const stake of state.ownershipStakes) if (stake.endYear === null) { const rows = activeStakesByOrganization.get(stake.organizationId) ?? []; rows.push(stake); activeStakesByOrganization.set(stake.organizationId, rows); }
  for (const organization of state.organizations.filter((row) => row.status !== "DISSOLVED")) { const stakes = activeStakesByOrganization.get(organization.organizationId) ?? []; if (stakes.reduce((sum, stake) => sum + stake.ownershipShareBps, 0) !== 10_000 || stakes.reduce((sum, stake) => sum + stake.controlShareBps, 0) !== 10_000) throw new Error(`Organization ${organization.organizationId} has invalid active shares`); }
...
export function advanceWorldOneYear(prior: WorldStateV5, context: V5EngineContext): AdvanceYearResult {
  ...
  const mark = (phase: string): void => {
    ...
    context.onPerformanceTiming?.({ scope: "ENGINE", worldKey: state.worldKey, year: state.year, phase, milliseconds: completedAt - phaseStartedAt });
    phaseStartedAt = completedAt;
  };
  mark("WORLD_TRANSACTIONAL_CANDIDATE");
  ...
  state = state.populationSlices === undefined || state.populationSlices.length === 0 && state.cohorts.length > 0 ? ensurePopulationSlicesV5(state, context.canonical) : state;
  mark("POPULATION_SLICE_PROCESSING_INITIAL");
  ...
  mark("SCHEDULED_CANONICAL");
  if (scheduled.events.length > 0) state = reconcilePublicPopulationSlicesV5(state, context.canonical);
  if (scheduled.events.some((event) => event.eventType === "SettlementFounded" || event.eventType === "StateMembershipChanged" || event.mutations.some((mutation) => mutation.mutationType === "STATE_MEMBERSHIP_CHANGED"))) { const routeReview = reconcileWorldRoutes(state, context.canonical, context.ownerInputs, context.mechanics); state = routeReview.state; events.push(...routeReview.events); namingRequests.push(...routeReview.namingRequests); }
  mark("POPULATION_SLICE_PROCESSING_SCHEDULED");
  const temporal = applyTemporalEventsV5(state); state = temporal.state; events.push(...temporal.events);
  mark("TEMPORAL");
  const resources = establishResourceGeographyV5(state, historicalContext); state = resources.state; events.push(...resources.events);
  mark("RESOURCE_GEOGRAPHY");
  const civic = updateCivicInstitutionsAndSecurityV5(state, historicalContext); state = civic.state; events.push(...civic.events);
  namingRequests.push(...civic.namingRequests);
  mark("INSTITUTIONS_SECURITY");
  ...
  mark("NATURAL_DEMOGRAPHY");
  state = reconcilePublicPopulationSlicesV5(state, context.canonical, { populationSlicesComplete: true, populationsRemainPositive: true });
  mark("POPULATION_SLICE_POST_DEMOGRAPHY_RECONCILIATION");
  state = applyPublicSliceGrowthModifiersV5(preDemography, state);
  mark("POPULATION_SLICE_GROWTH_MODIFIERS");
  ...
  if (structuralReviewRan) {
    ...
    metrics = deriveMetrics(state, context.canonical, context.mechanics);
    const mobilityIndexes = buildEphemeralWorldIndexesV5(state, context.canonical, { includePopulationSlices: false });
    const mobilityInputs = Object.fromEntries(state.settlements.map((settlement) => { const cells = mobilityIndexes.cohortsBySettlement.get(settlement.settlementId) ?? []; return [settlement.settlementId, { localOpportunity: metrics.localOpportunity[settlement.settlementId]!, institutionalAccess: accessBySettlement[settlement.settlementId]!, inequality: inequality(state, settlement.settlementId, cells), economicStrain: economicStrain(state, settlement.settlementId, cells) }]; }));
    const mobility = reviewAllSettlementsSocialMobilityV5(state, state.settlements, mobilityInputs, mobilityIndexes.cohortsBySettlement, context.mechanics); state = mobility.state; events.push(...mobility.events);
    state = reconcilePublicPopulationSlicesV5(state, context.canonical);
    mark("SOCIAL_MOBILITY");
    metrics = deriveMetrics(state, context.canonical, context.mechanics);
  ...
  const populationTotals = validatePopulationPartitionV5(state);
  validateState(state);
  validateSecurityForceOrganizationIntegrityV5(state);
  mark("AUDIT_VALIDATION");
  ...
  const result = { state, events: resequence(events), namingRequests: namingRequests.sort((a, b) => a.entityType.localeCompare(b.entityType) || a.entityId.localeCompare(b.entityId)), structuralReviewRan, migrationReviewRan, diagnosticObservations };
  mark("EVENT_RESEQUENCE");
  return result;
}
