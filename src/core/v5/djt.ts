...
export function executeDjtV5(state: WorldStateV5, canonical: CanonicalDataV5, variables: MechanicsVariablesV1, policy: DjtPolicyV5): { state: WorldStateV5; events: CausalEventV5[]; namingRequests: NamingRequestV5[] } {
  ...
  if (!working.states.some((politicalState) => politicalState.stateId === policy.innerwoodStateId)) {
    ...
    namingRequests.push({ requestId: `NAME_${innerwoodState.stateId}`, entityType: "STATE", entityId: innerwoodState.stateId, behavior: "BLOCKING", createdYear: state.year, nameEffectiveFromYear: state.year, worldKey: state.worldKey, namingComparisonGroupId: null, comparisonAuthorityRef: null, acceptedLabel: null, context: { world: state.worldKey, creationYear: state.year, causalReason: "DJT_STATE_CREATED", stateId: innerwoodState.stateId, djtYearAuthorityStatus: "UNRESOLVED_OWNER_AUTHORITY" } });
  }
