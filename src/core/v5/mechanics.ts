...
export function inequality(state: WorldStateV5, settlementId: string): Score1000 { return weightedMean([prosperityDispersion(state.cohorts.filter((cell) => cell.settlementId === settlementId)), 6000], [ownershipConcentration(state, settlementId), 4000]); }
export function economicStrain(state: WorldStateV5, settlementId: string): Score1000 { const settlement = state.settlements.find((row) => row.settlementId === settlementId)!; return weightedMean([1000 - settlementProsperity(state.cohorts.filter((cell) => cell.settlementId === settlementId)), 6000], [1000 - industryMean(settlement), 4000]); }
...
export function updateIndustry(state: WorldStateV5, canonical: CanonicalDataV5, variables: MechanicsVariablesV1, suppliedMetrics?: ReturnType<typeof deriveMetrics>): WorldStateV5 {
  ...
  return { ...state, settlements: state.settlements.map((settlement) => {
    const site = canonical.sites.find((row) => row.siteId === settlement.siteId)!; const population = settlementPopulation(state, settlement.settlementId); const cells = state.cohorts.filter((cell) => cell.settlementId === settlement.settlementId); const lowMid = cells.reduce((sum, cell) => sum + cell.tiers.LOW.population + cell.tiers.MID.population, 0n); const highMid = cells.reduce((sum, cell) => sum + cell.tiers.HIGH.population + cell.tiers.MID.population, 0n); const labor = population === 0n ? 0 : Number(divideRoundedAway(lowMid * 1000n, population)); const knowledge = population === 0n ? 0 : Number(divideRoundedAway(highMid * 1000n, population)); const capital = capitalAvailability(state, settlement.settlementId); const trade = metrics.tradeAccess[settlement.settlementId]!;
...
export function organizationFormationInputs(state: WorldStateV5, canonical: CanonicalDataV5, variables: MechanicsVariablesV1, accessBySettlement: Readonly<Record<string, Score1000>>, institutionEffectivenessByState: Readonly<Record<string, Score1000>>, suppliedMetrics?: ReturnType<typeof deriveMetrics>): { proposals: OrganizationFormationProposal[]; contextsByOrganization: Record<string, OrganizationFormationContext> } {
  ...
  const contextFor = (settlementId: string, sectorId: SectorId): OrganizationFormationContext => { const settlement = state.settlements.find((row) => row.settlementId === settlementId)!; const politicalState = state.states.find((row) => row.stateId === settlement.stateId)!; const familyNetwork = state.families.filter((family) => family.status === "ACTIVE" && family.homeSettlementId === settlementId).reduce((maximum, family) => Math.max(maximum, familyNetworkStrength(state, family.familyId)), 0); return { settlementId, sectorId, sectorStrength: settlement.sectorStrengths[sectorId], capitalAvailability: capitalAvailability(state, settlementId), tradeAccess: metrics.tradeAccess[settlementId]!, enforcement: enforcementStrength(institutionEffectivenessByState[settlement.stateId] ?? 500, politicalState.legitimacy, 1000 - settlement.unrest), politicalExclusion: 1000 - (accessBySettlement[settlementId] ?? 500), unrest: settlement.unrest, familyNetwork, concentration: ownershipConcentration(state, settlementId) }; };
