...
export function organizationPerformanceTarget(type: OrganizationType, context: OrganizationFormationContext): Score1000 {
  if (type === "CORPORATION") return weightedMean([context.sectorStrength, 3000], [context.capitalAvailability, 2500], [context.tradeAccess, 2000], [context.enforcement, 1500], [1000 - context.unrest, 1000]);
  if (type === "GUILD") return weightedMean([context.sectorStrength, 3500], [context.capitalAvailability, 1500], [context.tradeAccess, 2000], [context.familyNetwork, 1500], [1000 - context.unrest, 1500]);
  return weightedMean([sectorRentScore(context.sectorStrength, context.concentration), 3000], [context.politicalExclusion, 2500], [1000 - context.enforcement, 2500], [context.familyNetwork, 1000], [context.unrest, 1000]);
}
