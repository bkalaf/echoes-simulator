...
import type { V5AtomicYearSnapshot } from "./runner.js";
...
export interface PersistedV5DiagnosticInput {
  ...
  onPersistedAtomicYear?: (snapshot: V5AtomicYearSnapshot, runId: string) => void;
  divergenceDiagnosticYears?: readonly number[];
}

export function shouldBuildDivergenceDiagnosticV5(input: { year: number; targetYear: number; intervalYears: number; explicitYears: ReadonlySet<number>; decisionBarrier: boolean }): boolean {
  if (!Number.isSafeInteger(input.intervalYears) || input.intervalYears <= 0) throw new Error("divergenceDiagnosticIntervalYears must be positive");
...
export function runPersistedV5Diagnostic(input: PersistedV5DiagnosticInput): {
  ...
  const persistSnapshot = (snapshot: Parameters<NonNullable<Parameters<typeof runV5History>[0]["onAtomicYear"]>>[0]): void => {
    input.store.withV5AtomicYearTransaction(() => {
    for (const world of WORLDS) {
      const yearEvents = snapshot.yearEvents[world];
      let startedAt = performance.now();
      const canonicalYearEvents = yearEvents.map((event) => canonicalJson(event));
      historyHashes[world] = extendV5EventHistoryHashFromCanonicalJson(historyHashes[world], canonicalYearEvents);
      input.onPerformanceTiming?.({ scope: "PERSISTENCE", worldKey: world, year: snapshot.year, phase: "EVENT_HISTORY_HASH", milliseconds: performance.now() - startedAt, rows: yearEvents.length });
      startedAt = performance.now();
      input.store.appendV5CausalEvents(runId, yearEvents, canonicalYearEvents);
      input.onPerformanceTiming?.({ scope: "PERSISTENCE", worldKey: world, year: snapshot.year, phase: "EVENT_PERSISTENCE", milliseconds: performance.now() - startedAt, rows: yearEvents.length });
      startedAt = performance.now();
      input.store.saveV5NamingRequests(runId, snapshot.yearNamingRequests[world]);
      input.onPerformanceTiming?.({ scope: "PERSISTENCE", worldKey: world, year: snapshot.year, phase: "NAMING_PERSISTENCE", milliseconds: performance.now() - startedAt, rows: snapshot.yearNamingRequests[world].length });
      startedAt = performance.now();
      input.store.mergeV5DiagnosticObservations(runId, snapshot.yearDiagnosticObservations[world]);
      input.onPerformanceTiming?.({ scope: "DIAGNOSTIC", worldKey: world, year: snapshot.year, phase: "DIAGNOSTIC_PERSISTENCE", milliseconds: performance.now() - startedAt, rows: snapshot.yearDiagnosticObservations[world].length });
      if (snapshot.checkpointDue) input.store.saveV5Checkpoint(runId, snapshot.states[world], historyHashes[world], input.onPerformanceTiming, operational.checkpointCompressionLevel);
    }
    if (shouldBuildDivergenceDiagnosticV5({ year: snapshot.year, targetYear, intervalYears: operational.divergenceDiagnosticIntervalYears, explicitYears: new Set(input.divergenceDiagnosticYears ?? []), decisionBarrier: false })) {
      let startedAt = performance.now();
      ...
      input.onPerformanceTiming?.({ scope: "DIAGNOSTIC", worldKey: "MULTIWORLD", year: snapshot.year, phase: "DIVERGENCE_REPORT", milliseconds: performance.now() - startedAt });
      startedAt = performance.now();
      ...
      input.onPerformanceTiming?.({ scope: "DIAGNOSTIC", worldKey: "MULTIWORLD", year: snapshot.year, phase: "DIVERGENCE_PERSISTENCE", milliseconds: performance.now() - startedAt, rows: updatedTraces.changed.length });
    }
  ...
  try {
    ...
    const pauseCheckpoint = (states: Readonly<Record<WorldKey, Parameters<typeof input.store.saveV5Checkpoint>[1]>>): void => { for (const world of WORLDS) input.store.saveV5Checkpoint(runId, states[world], historyHashes[world], input.onPerformanceTiming, operational.checkpointCompressionLevel); };
    const result = runV5History({ canonical, ownerInputs, mechanics: configuration.mechanics, operational: configuration.operational, diagnostic: configuration.diagnostic, normalizedSeed: input.normalizedSeed, mode: "DIAGNOSTIC", throughYear: targetYear, scheduledTransactions, stopAtBlockingNaming: input.namingMode !== "UNATTENDED_CAUSAL_BENCHMARK", interactiveNamingEnabled: interactive, retainHistory: false, onBootstrap: persistSnapshot, onAtomicYear: persistSnapshot, onPauseCheckpoint: pauseCheckpoint, acceptedDerogatoryDecisionBatches: [], onPerformanceTiming: input.onPerformanceTiming, onPolicyBlocker: (blocker) => input.store.saveV5PolicyBlocker(runId, blocker), onDerogatoryDecisionBarrier: (batch) => input.store.saveV5DerogatoryDecisionBatch(runId, batch) });
...
export function resumePersistedV5Run(input: { store: SimulatorStore; resourceDirectory: string; runId: string; onPerformanceTiming?: V5PerformanceTimingObserver; onPersistedAtomicYear?: (snapshot: V5AtomicYearSnapshot, runId: string) => void; divergenceDiagnosticYears?: readonly number[] }): { runId: string; status: "COMPLETE" | "WAITING_FOR_NAMING" | "WAITING_FOR_POLICY_AUTHORITY" | "WAITING_FOR_DEROGATORY_DECISIONS"; currentYear: number; divergence: ReturnType<typeof buildDivergenceReport> } {
  ...
  // The checkpoint and all three worlds' events are committed in one atomic
  // SQLite transaction. Reuse that checkpoint's chain hash on continuation;
  // replay acceptance independently recomputes the ledger hash by segment.
  const historySummaries = Object.fromEntries(WORLDS.map((world) => [world, { eventHistoryHash: checkpoints[world]!.eventHistoryHash, eventCount: input.store.v5CausalEventCount(input.runId, world, states[world].year) }])) as Record<WorldKey, { eventHistoryHash: string; eventCount: number }>;
  for (const world of WORLDS) if (input.store.v5CausalEventCount(input.runId, world) !== historySummaries[world].eventCount) throw new Error(`V5_NONATOMIC_EVENT_TAIL run=${input.runId} world=${world} checkpointYear=${states[world].year}`);
  ...
  const persistSnapshot = (snapshot: Parameters<NonNullable<Parameters<typeof continueV5History>[0]["onAtomicYear"]>>[0]): void => {
    ...
    for (const world of WORLDS) {
      let startedAt = performance.now();
      const canonicalYearEvents = snapshot.yearEvents[world].map((event) => canonicalJson(event));
      historySummaries[world].eventHistoryHash = extendV5EventHistoryHashFromCanonicalJson(historySummaries[world].eventHistoryHash, canonicalYearEvents);
      historySummaries[world].eventCount += snapshot.yearEvents[world].length;
      input.onPerformanceTiming?.({ scope: "PERSISTENCE", worldKey: world, year: snapshot.year, phase: "EVENT_HISTORY_HASH", milliseconds: performance.now() - startedAt, rows: snapshot.yearEvents[world].length });
      startedAt = performance.now();
      input.store.appendV5CausalEvents(input.runId, snapshot.yearEvents[world], canonicalYearEvents);
      input.onPerformanceTiming?.({ scope: "PERSISTENCE", worldKey: world, year: snapshot.year, phase: "EVENT_PERSISTENCE", milliseconds: performance.now() - startedAt, rows: snapshot.yearEvents[world].length });
      startedAt = performance.now();
      ...
      input.onPerformanceTiming?.({ scope: "DIAGNOSTIC", worldKey: world, year: snapshot.year, phase: "DIAGNOSTIC_PERSISTENCE", milliseconds: performance.now() - startedAt, rows: snapshot.yearDiagnosticObservations[world].length });
      if (snapshot.checkpointDue) input.store.saveV5Checkpoint(input.runId, snapshot.states[world], historySummaries[world].eventHistoryHash, input.onPerformanceTiming, manifest!.operationalConfig.checkpointCompressionLevel);
    }
    if (shouldBuildDivergenceDiagnosticV5({ year: snapshot.year, targetYear: manifest!.targetYear, intervalYears: manifest!.operationalConfig.divergenceDiagnosticIntervalYears, explicitYears: new Set(input.divergenceDiagnosticYears ?? []), decisionBarrier: false })) {
      let startedAt = performance.now();
