...
const effectiveBreeds = batchIds.flatMap((batchId) => readJsonLines<EffectiveBreedSemantics>(resolve(batchesRoot, batchId, "effective_breed_preview.jsonl"))).sort((a, b) => a.breedId.localeCompare(b.breedId));
...
exact("Effective civic Breed", civicIdentities.map((row) => row.breedId), effectiveBreeds.map((row) => row.breedId));
...
const manifest = {
  ...
  filename: basename(outputZip), personalityPolicyRef: PERSONALITY_DIMENSION_POLICY,
  counts: { breeds: 2056, civicBreeds: 1773, pets: 283, researchUnits: 1219, regionBatches: 29, auditShards: 7, sources: sources.length, citations: citations.length, evidence: evidence.length, inheritanceEdges: inheritanceEdges.length, effectiveCivicBreeds: effectiveBreeds.length },
...
writeFileSync(resolve(outputDirectory, "consolidation_report.json"), `${JSON.stringify({ status: "PASS", pureMaterialization: true, evidenceCreatedByConsolidation: 0, zipReopenedAndVerified: true, counts: manifest.counts }, null, 2)}\n`);
