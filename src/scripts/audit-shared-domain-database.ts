...
try {
  const [identity, migrations, counts, authority, reconciliation, policyStatuses, sharedTables] = await Promise.all([
    pool.query<{ databaseName: string }>(`SELECT current_database() AS "databaseName"`),
    pool.query<{ migration_name: string }>(`SELECT migration_name FROM "_prisma_migrations" WHERE finished_at IS NOT NULL AND migration_name IN ('20260829120000_canonical_geography_and_political_alignment','20260829210000_master_remediation_v56','20260830010000_canonical_domain_reconciliation') ORDER BY migration_name`),
    pool.query<{ breeds: number; boundBreeds: number; deities: number; pois: number; policyDefinitions: number; lockedAuthorities: number }>(`SELECT (SELECT COUNT(*)::int FROM "Breed") AS breeds, (SELECT COUNT("primaryDeityId")::int FROM "Breed") AS "boundBreeds", (SELECT COUNT(*)::int FROM "Deity") AS deities, (SELECT COUNT(*)::int FROM "PointOfInterest") AS pois, (SELECT COUNT(*)::int FROM "OwnerPolicyDefinition") AS "policyDefinitions", (SELECT COUNT(*)::int FROM "LockedOwnerAuthority") AS "lockedAuthorities"`),
    ...
    pool.query<{ authorityId: string; status: string; unexplainedDifferenceCount: number; sourceValueCount: number; importedValueCount: number }>(`SELECT DISTINCT ON ("authorityId") "authorityId", status, "unexplainedDifferenceCount", "sourceValueCount", "importedValueCount" FROM "CanonicalMigrationReconciliation" ORDER BY "authorityId", "activatedAt" DESC NULLS LAST, "updatedAt" DESC`),
  ...
  const artifact = {
    ...
    canonicalDomainMigration: { ownerApprovalRequired: false, domains: reconciliation.rows, unexplainedMigratedValues: reconciliation.rows.reduce((sum, value) => sum + value.unexplainedDifferenceCount, 0) },
    ...
    acceptance: {
      ...
      existingCanonicalDatabaseReused: connection.sharedCanonicalDatabase && migrations.rows.length === 3 ? "PASS" : "FAIL",
      ...
      databaseInfrastructureState: preflight.state,
      blanketSimulatorCanonicalV5OwnerApproval: "REMOVED",
      migratedCanonicalValuesReconciled: reconciliation.rows.length === 10 && reconciliation.rows.every((value) => value.status === "RECONCILED" && value.unexplainedDifferenceCount === 0 && value.sourceValueCount === value.importedValueCount) ? "PASS" : "FAIL",
      unexplainedMigratedValues: reconciliation.rows.reduce((sum, value) => sum + value.unexplainedDifferenceCount, 0),
      breedCatalogAvailableIndependently: reconciliation.rows.some((value) => value.authorityId === "BREED_CATALOG_V5" && value.status === "RECONCILED") ? "PASS" : "FAIL",
      atlasAvailableIndependently: row.pois > 0 ? "PASS" : "FAIL",
      unresolvedDomainAuthorities: "CAPABILITY_SPECIFIC",
      unapprovedPolicyBlockingScope: "FIRST_CONSUMER_ONLY",
  ...
  process.stdout.write(`${JSON.stringify({ status: "SHARED_POSTGRES_AUDIT_WRITTEN", preflightState: preflight.state, diagnosticCode: preflight.diagnosticCode, appliedSimulatorMigrations: migrations.rows.length, reconciledCanonicalDomains: reconciliation.rows.length, unexplainedMigratedValues: artifact.canonicalDomainMigration.unexplainedMigratedValues, policyDefinitions: row.policyDefinitions, lockedAuthorities: row.lockedAuthorities, breedRows: row.breeds, boundPrimaryDeityRows: row.boundBreeds, deityRows: row.deities, atlasPoiRows: row.pois, acceptance: artifact.acceptance }, null, 2)}\n`);
} finally {
