...
export function AtlasView({ data, world, selectedPoiId, setWorld, selectPoi }: { data: AtlasData | null; world: World; selectedPoiId: string | null; setWorld: (world: World) => void; selectPoi: (poiId: string) => void }): React.JSX.Element {
    ...
    {!data ? <section className="panel vertical"><h2>Loading master Atlas…</h2></section> : <div className="atlas-layout">
      <section className="atlas-stage" aria-label={`Master Atlas layers for ${world}`}>
        ...
        <svg className="route-overlay" viewBox="0 0 1000 500" preserveAspectRatio="none">{(data.routes ?? []).filter((route) => route.worlds[world]?.active).flatMap((route) => {
          ...
          if (layers.routes && realization?.primaryMode !== "NONE") elements.push(<line key={`${route.corridorId}-physical`} role="button" tabIndex={0} aria-label={`${route.corridorId} route overlay`} className={`route-line mode-${realization?.primaryMode.toLowerCase()}${selectedRouteId === route.corridorId ? " selected" : ""}`} x1={x1} y1={y1} x2={x2} y2={y2} onClick={() => setSelectedRouteId(route.corridorId)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelectedRouteId(route.corridorId); }}/>);
          if (layers.portals && route.portalCapability) elements.push(<line key={`${route.corridorId}-portal`} role="button" tabIndex={0} aria-label={`${route.corridorId} portal link overlay`} className={`route-line mode-portal${selectedRouteId === route.corridorId ? " selected" : ""}`} x1={x1} y1={y1} x2={x2} y2={y2} onClick={() => setSelectedRouteId(route.corridorId)}/>);
