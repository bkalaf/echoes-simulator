...
export function RoutesView({ data }: { data: AtlasData | null }): React.JSX.Element {
  ...
  const routes = useMemo(() => (data?.routes ?? []).filter((route) => {
    ...
    if (filter === "UNNAMED") return WORLDS.some((world) => route.worlds[world]?.active && ["PENDING", "NAMEABLE"].includes(route.worlds[world]?.nameStatus ?? ""));
...
function RouteDetail({ route, all }: { route?: AtlasRoute; all: AtlasRoute[] }): React.JSX.Element {
  ...
  return <section className="detail route-detail"><p className="eyebrow">ROUTE DETAIL</p><h2>{route.regionA.regionName} ↔ {route.regionB.regionName}</h2><dl><div><dt>Corridor</dt><dd>{route.corridorId}</dd></div><div><dt>Directionality</dt><dd>{route.canonicalDirectionality}</dd></div><div><dt>Portal capability</dt><dd>{route.portalCapability ? "Yes" : "No authority"}</dd></div><div><dt>Semantic readiness</dt><dd>{route.semanticReadiness}</dd></div><div><dt>Classification authority</dt><dd>{route.classificationAuthorityVersion ?? "OWNER DECISION REQUIRED"}</dd></div><div><dt>Owner evidence</dt><dd>{route.ownerEvidenceRef ?? "—"}</dd></div></dl>{WORLDS.map((world) => { const realization = route.worlds[world]; return <article className="route-world" key={world}><h3>{world}</h3><dl><div><dt>Name</dt><dd>{realization?.name ?? "Unresolved"}</dd></div><div><dt>Route ID</dt><dd>{realization?.routeId ?? "Inactive"}</dd></div><div><dt>Effective mode / class</dt><dd>{realization?.primaryMode ?? route.primaryMode} · {realization?.infrastructureClass ?? route.infrastructureClass}</dd></div><div><dt>Persisted historical mode / class</dt><dd>{realization?.persistedPrimaryMode ?? "—"} · {realization?.persistedInfrastructureClass ?? "—"}</dd></div><div><dt>Trade</dt><dd>{realization?.tradeDesignation ? "Trade designated" : "Not trade designated"}</dd></div><div><dt>Status</dt><dd>{realization?.nameStatus ?? "INACTIVE"}</dd></div><div><dt>Established</dt><dd>{realization?.establishedYear ?? "—"}</dd></div><div><dt>Endpoint context</dt><dd>{realization?.endpointSettlements ? `${Object.keys(realization.endpointSettlements).length} Region endpoint groups` : "Unavailable until naming sync"}</dd></div><div><dt>Sovereign context</dt><dd>{realization?.worldSovereignFaction ?? "—"} · formal crown allegiance {realization?.formalCrownAllegianceAuthority ? "present" : "not asserted"}</dd></div><div><dt>Naming anchor</dt><dd>{realization?.preferredNamingAnchor ?? "None"}</dd></div><div><dt>Name provenance</dt><dd>{realization?.nameProvenance ?? "Pending owner naming"}</dd></div></dl></article>; })}<p>{differences > 1 ? "Cross-world route-name difference is present." : "Cross-world labels are identical or uniformly unresolved."} Inventory: {all.length} corridors.</p></section>;
}
