...
interface Preflight { structuralStatus: string; canonicalReady: boolean; counts: Record<string, number>; coverage: Record<string, { resolved: number; unresolved: number }>; activeIssues: Issue[]; supplementalResearch: { filename: string; rows: number; precedence: string } | null; }
...
function App(): React.JSX.Element {
  ...
  async function validateInputs(): Promise<void> {
    ...
    if (!directory) return;
    setBusy("Validating");
    try { await window.eidolonSimulator.validateInputs(directory); setMessage("Input structure passed; canonical blockers remain visible"); }
