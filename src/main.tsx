...
function App(): React.JSX.Element {
  ...
  const content = (() => {
    ...
    if (selected === "Atlas") return <AtlasView data={atlasData} world={world} selectedPoiId={atlasPoiId} selectedRouteId={atlasRouteId} setWorld={setWorld} selectPoi={setAtlasPoiId} selectRoute={setAtlasRouteId}/>;
...
function HistoricalRows({ title, world, year, rows, empty }: { title: string; world: World; year: number; rows: readonly object[]; empty: string }): React.JSX.Element {
  return <section className="panel vertical"><p className="eyebrow">{world} · YEAR {year}</p><h2>{title}</h2>{rows.length === 0 ? <p>{empty}</p> : <div className="table-wrap"><table><thead><tr><th>Stable identity</th><th>Historical data</th></tr></thead><tbody>{rows.slice(0, 250).map((value, index) => { const row = value as Record<string, unknown>; const identity = Object.entries(row).find(([key]) => /(?:Id|ID)$/.test(key))?.[1] ?? `${title}-${index}`; return <tr key={String(identity)}><td>{String(identity)}</td><td><code>{JSON.stringify(row, (_key, item) => typeof item === "bigint" ? item.toString() : item)}</code></td></tr>; })}</tbody></table></div>}</section>;
}
