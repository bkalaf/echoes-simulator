...
function App(): React.JSX.Element {
  ...
  const content = (() => {
    if (selected === "Runs") return <>
      <section className="cards"><Stat label="DATABASE" value={snapshot.domainDatabasePreflight?.state ?? "CHECKING"} note={snapshot.domainDatabasePreflight?.diagnosticCode ?? "Checking shared PostgreSQL"}/><Stat label="RUN STATUS" value={snapshot.v5Run ? operator.runState : "NO V5 RUN"} note={snapshot.v5Run ? manifest?.runId ?? "—" : "Existing SQLite history remains independent"}/><Stat label="NAMING REQUESTS" value={snapshot.v5Run ? manifest?.namingJobCount ?? 0 : "—"} note="BATCHED requests do not pause history"/></section>
      ...
      {snapshot.capabilityReadiness && <section className="panel vertical"><p className="eyebrow">CAPABILITY READINESS</p><h2>Canonical migration {snapshot.capabilityReadiness.canonicalDomainMigration.status}</h2><p>{snapshot.capabilityReadiness.canonicalDomainMigration.unexplainedDifferenceCount} unexplained migrated values. Unresolved authority blocks only its listed causal consumer.</p><div className="table-wrap"><table><thead><tr><th>Capability</th><th>Status</th><th>Scope</th><th>Evidence</th></tr></thead><tbody>{snapshot.capabilityReadiness.capabilities.map((capability) => <tr key={capability.capabilityId}><td>{capability.capabilityId}</td><td>{capability.status}</td><td>{capability.blockingScope === "NONE" ? "Available" : "First causal consumer only"}</td><td>{capability.detail}</td></tr>)}</tbody></table></div></section>}
    </>;
    ...
    if (selected === "Diagnostics") return <><section className="cards"><Stat label="DATABASE" value={snapshot.domainDatabasePreflight?.state ?? "CHECKING"} note={snapshot.domainDatabasePreflight?.diagnosticCode ?? "Infrastructure preflight"}/><Stat label="CANONICAL MIGRATION" value={snapshot.capabilityReadiness?.canonicalDomainMigration.status ?? "UNAVAILABLE"} note={`${snapshot.capabilityReadiness?.canonicalDomainMigration.unexplainedDifferenceCount ?? "—"} unexplained values`}/><Stat label="LEGACY V4" value={operator.semanticAuthorityLabel} note="Retained for historical access"/></section><section className="panel vertical"><p className="eyebrow">POINT-OF-USE CAUSAL AUTHORITY</p><h2>{snapshot.v5CanonicalReadiness?.ready ? "Candidate configuration complete" : `${snapshot.v5CanonicalReadiness?.missing.length ?? 0} candidate policy revisions remain`}</h2><p>Pending policies do not change database health or hide independent product pages. Each unapproved revision fails closed only when its causal consumer is reached.</p></section><section className="panel"><div><p className="eyebrow">LEGACY V4</p><h2>Archived V4 run controls</h2><p>V4 persistence remains accessible here but is excluded from the normal V5 run strip.</p></div><div className="legacy-actions"><button onClick={() => void runDiagnostic()} disabled={busy !== null || !operator.canRunDiagnostic}>{busy ?? "RUN LEGACY V4 DIAGNOSTIC"}</button><button onClick={() => void runCanonical()} disabled={busy !== null || !operator.canRunCanonical}>{busy ?? "RUN LEGACY V4 CANONICAL"}</button></div></section>{snapshot.runs.some((run) => !run.isV5) && <section className="panel vertical"><p className="eyebrow">LEGACY V4 RUNS</p><div className="tabs">{snapshot.runs.filter((run) => !run.isV5).map((run) => <button key={run.runId} onClick={() => void selectRun(run.runId)}>{run.mode} · {run.status} · {run.currentYear}</button>)}</div></section>}</>;
...
function DomainAuthorityBlocked({ preflight, domain }: { preflight?: DomainDatabasePreflight; domain: string }): React.JSX.Element {
  ...
  return <section className="panel vertical" role="alert"><p className="eyebrow">POSTGRESQL INFRASTRUCTURE REQUIRED</p><h2>{domain} unavailable</h2><p>{preflight.connectionLabel ? `Connected — ${preflight.connectionLabel} · ` : ""}{preflight.state} · {preflight.diagnosticCode}</p><p>Open Runs and use the displayed doctor, migrate, or retry control. Existing SQLite history remains readable; no runtime filesystem authority fallback is permitted.</p></section>;
}
